...
**Recent Moments (right box top/middle/bottom) - also fully automatic, for content.** Every image in `media/<campus>/recent/` is eligible; all three boxes swap to a new photo simultaneously every `recentRoundDuration` seconds. Within one round the three photos are always distinct as long as the pool has >= 3 photos (guaranteed by construction - a fresh Fisher-Yates shuffle each round, not just "unlikely to collide"); below 3 photos it falls back to repeats rather than leaving a box blank. Photos are cropped to fill (`object-fit: cover`), not letterboxed. Videos in `recent/` are ignored - these are photo-only slots.

**Upcoming events (left box) - text cards, not media.** Reads the first `*_upcoming_events.json` file found directly in `media/<campus>/` (e.g. `media/milpitas/mp_upcoming_events.json`) - not a Drive-synced subfolder, this file is written there by whatever external process generates it (out of scope here; this just reads it). One event shown at a time, in the order given in the JSON (not shuffled - source order is assumed meaningful), advancing every `eventsDuration` seconds. White background, black text, per spec. The event schema is deliberately loose (a plain `date`, or `date_range` + `recurrence`, or a `dates` array, plus optional `location`/`address`/`notes`/`cost`/`speakers`/`special_guest`/`doors_open`) since it comes from an upstream newsletter scrape - `formatSchedule()`/`renderEventCard()` in `public/app.js` render whatever fields happen to be present rather than assuming a fixed shape. Missing file or no events found in it just leaves the box blank, doesn't break anything else.
...
- **Right box top/middle/bottom** - three separate simultaneous slots ("Recent Moments"), sourced from Drive's `recent/` subfolder, synchronized round-robin - not a single rotating strip (wired up)
- **Left box** - upcoming events text cards, sourced from a local `*_upcoming_events.json` in the campus directory, one at a time, white background/black text (wired up)
